...
export type Article = {
  ...
  // Rich fields – any can be string (multi-line) or string[]
  rca?: string | string[];
  prechecks?: string | string[];
  steps?: string | string[];
  validation?: string | string[];

  tags?: string[];       // e.g., ["DEV","TEST","UAT","PROD","AR"]
  lastUpdated?: string;
  severity?: string;     // e.g., "Low", "Medium", "High"

  links?: LinkItem[];    // e.g., [{label:"Oracle Doc", url:"https://..."}]
};
...
export default function ArticleGrid({ articles }: Props) {
  ...
  return (
    <>
      ...
      {/* Centered modal with full details in your sample format */}
      {open && active ? (
        <Modal onClose={closeModal} title={active.title}>
          ...
          <ArticleSection
            title="Pre-checks"
            content={active.prechecks}
            listType="bulleted"
          />

          <ArticleSection
            title="Resolution Steps"
            content={active.steps}
            listType="numbered"
          />
